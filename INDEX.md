...
18. **ETRG-9_sigma_calibration.md** — Gate 2, third item: σ = ħ/k_BT at equilibrium — scale forced by KMS, O(1) normalization stipulated (referee F1); the tick-time convergence is suggestive, not a theorem (F4); Connes–Rovelli cited. **Addendum: the Barontini cross-check executed — the experiment's σ is a conventional unit (resolution 3 supported); the physical content is the Erker bound, with σ as its saturation value (the ideal-clock rate); the equality-vs-inequality mismatch dissolved.**
